...
class qiwiPayment extends waPayment implements waIPaymentCancel, waIPaymentRefund
{
    ...
    public function payment($payment_form_data, $order_data, $auto_submit = false)
    {
        ...
        switch ($this->protocol) {
            case self::SOAP:
                $result = $this->soapPayment($payment_form_data, $order_data, $auto_submit);
                break;
            case self::REST:
    ...
    private function restPayment($payment_form_data, $order_data, $auto_submit = false)
    {
        ...
        $transaction_data = $transaction_model->getByField(array(
            'app_id'   => $this->app_id,
            'plugin'   => $this->id,
            'order_id' => $this->order_id,
        ), 'id');
        ...
        if (!empty($transaction_data['native_id'])) {
            if ($transaction_data['state'] == self::STATE_CAPTURED) {
                ...
                $raw_status = $transaction_data_model->getByField(array(
    ...
    private function restCallbackHandler($data)
    {
        ...
        $signature = waRequest::server($field);
         ...
         */
        self::log($this->id, var_export(compact('data', 'signature', 'sign', 'server'), true));
        $result_code = 300;
    ...
    private function restQuery($params, $data = array())
    {
        ...
        if (function_exists('curl_init')) {
            if ($ch = curl_init()) {
                ...
                @curl_setopt($ch, CURLOPT_USERPWD, "{$this->api_login}:{$this->api_password}");
            ...
            if ($socket = fsockopen('ssl://'.$host, 443, $errorno, $errorstr, $timeout)) {
                ...
                $out .= "Authorization: Basic ".urlencode(base64_encode("{$this->api_login}:{$this->api_password}"))."\r\n";
    ...
    private function soapPayment($payment_form_data, $order_data, $auto_submit = false)
    {
     ...
     */
    private function updateBill($login = null, $password = null, $txn = null, $status = null)
    {
    ...
    private function getSign($transaction_raw_data)
    {
        ...
        return hash_hmac('sha1', implode('|', $string), $this->api_password);
    }
