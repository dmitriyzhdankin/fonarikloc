        ...
        foreach ($rows as $row) {
            if (!$row['sku_id']) {
                ...
                if (!$row['status']) {
                    ...
                    // update price
                    $services[$row['service_id']]['variants'][$row['service_variant_id']]['price'] = ceil($row['price']);
                }
            } else {
                if (!$row['status']) {
                    $skus_services[$row['sku_id']][$row['service_id']][$row['service_variant_id']] = false;
                } else {
                    $skus_services[$row['sku_id']][$row['service_id']][$row['service_variant_id']] =  ceil($row['price']);
                }
            }
        }
