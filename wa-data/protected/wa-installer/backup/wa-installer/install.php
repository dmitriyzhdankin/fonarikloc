...
<!--
<p>{$t->_('Extracting Webasyst arhcive...')}</p>
-->
...
HTML;
    ...
    case 1:{
        //download list of system requiriments
        try{
            if(!file_exists($init_path)){
                throw new Exception("File <b>wa-installer/lib/init.php</b> not found");
            }
            ...
            if($passed){
                try {
                    if($check_latest){
                        ...
                        $fw_items = $installer_apps->getSystemList();
...
if($checked){
    ...
    case 4:{
        ...
        if(!file_exists($config_path) && ($fp = fopen($config_path,'w'))) {
            $config_content = <<<PHP
<?php
...
<div class="i-progress-indicator">';
    for ($i=1;$i<$steps_count;$i++){
        $progress .= '<span id="i-progress-step-'.$i.'" class="'.($i<$step?'passed':( $i==$step ? 'current' : 'next' )).'" title="'.$steps[$i]['title'].'">'.($i).'</span>';
    }
