$.wa_blog = $.extend(true, $.wa_blog,
{
	plugins_category : {
		...
		ajaxInit : function() {
			var self = this;
			$("#b-plugins-categories .b-category-add").live('click.plugins_category', function(eventObject) {
						return self.addHandler.apply(self,[this,eventObject]);
			});
		// inline edit
			$('#b-plugins-categories .b-category-edit').live('click.plugins_category', function(eventObject) {
				return self.editHandler.apply(self,[this,eventObject]);
	});
			$('#b-plugins-categories .b-category-delete').live('click.plugins_category', function(eventObject) {
				return self.deleteHandler.apply(self,[this,eventObject]);
	});
			$('#b-plugins-categories .b-plugins-category-icon').live('change.plugins_category', function(eventObject) {
		...
		submitHandler : function(element) {
			...
			$.ajax({
				...
				success : function(response) {
					$.wa_blog.plugins.directLoad(response);
					self.makeSortable();
				},
